...
export default function Home() {
  ...
  const handleSend = () => {
    ...
    navigate('/assistant', { state: { draftMessage: text } });
  };
    ...
    <div>
      ...
      <section className="max-w-6xl mx-auto px-4 md:px-6 pt-6 pb-10 text-center">
        ...
        >
          Your Offline AI
          <br />
          <span className="text-emergency">Emergency Companion</span>
        </motion.h1>

        <div className="flex justify-center my-8">
          <AIOrb size={200} listening={speech.listening} />
        </div>
        ...
        >
          <input style={{ color: '#EDEAE3' }}
            type="text"
            value={speech.listening && speech.interimTranscript ? `${query} ${speech.interimTranscript}` : query}
            onChange={(e) => setQuery(e.target.value)}
            onKeyDown={(e) => e.key === 'Enter' && handleSend()}
            placeholder="Describe your emergency…"
            className="flex-1 bg-transparent outline-none placeholder:text-subtext text-sm md:text-base"
          />
          {speech.listening && <VoiceWave active />}
          <button
            onClick={handleMic}
          ...
          <Link to="/scan-injury" aria-label="Open camera to scan an injury" className="p-2.5 rounded-full bg-white/8 hover:bg-white/15 transition-colors">
            <Camera size={17} />
          </Link>
          <button onClick={handleSend} aria-label="Send" className="p-2.5 rounded-full bg-emergency hover:brightness-110 transition shadow-glow">
            <Send size={17} />
          </button>
        </motion.div>
        {speech.error && <p className="text-emergency text-xs mt-2">{speech.error}</p>}
      </section>
          ...
          <div className="flex flex-wrap items-center justify-between gap-6">
            ...
            <div className="flex items-center gap-6">
              <ProgressRing
                ...
                color={chatModel.status === 'ready' ? '#4C9A6A' : '#4A8C8C'}
                label="Chat model"
                size={72}
                stroke={7}
              />
              <ProgressRing
                ...
                color={visionModel.status === 'ready' ? '#4C9A6A' : '#7A7699'}
                label="Vision model"
                size={72}
                stroke={7}
              />
      ...
      <section className="max-w-6xl mx-auto px-4 md:px-6 grid grid-cols-1 md:grid-cols-3 gap-5">
        ...
        <GlassCard className="p-5 col-span-1">
          ...
          {battery.status === 'success' ? (
            <div className="flex items-center gap-4">
              <ProgressRing value={battery.level} color="#C4903D" size={64} stroke={6} />
