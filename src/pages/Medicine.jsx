...
const stockColors = { 'In stock': '#22C55E', 'Low stock': '#F59E0B', 'Out of stock': '#FF4D4D' };
...
export default function Medicine() {
  ...
  const addItem = async (e) => {
    ...
    if (!name.trim()) return;
    try {
      const item = await api.addMedicine({ name, use: use || 'General use', stock: 'In stock', color: '#22C55E' });
