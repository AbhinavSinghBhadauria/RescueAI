...
function useModelWorker(workerFactory) {
  ...
  const ensureWorker = useCallback(() => {
    ...
    // Native worker-level failures (module load errors, CORS, uncaught
    // exceptions before a message is posted) never reached onmessage,
    // which left `load()` hanging forever with no feedback. Handle them.
    worker.onerror = (err) => {
      console.error('Model worker error:', err);
      setStatus('error');
      pendingRef.current?.reject?.(new Error(err?.message || 'Worker failed to load — see browser console.'));
      pendingRef.current = null;
    };
